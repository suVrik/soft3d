...
VertexBuffer vertex_buffer = { 55668, vertex_data };

#include "dog_texture.h"

ColorBuffer texture_buffer = { 1024, 1024, (Color*)texture_data };
...
void potato_init() {
	...
	for (size_t i = 0; i < 1024; i++) {
		for (size_t j = 0; j < 1024; j++) {
			unsigned char temp = texture_data[i * 1024 * 4 + j * 4];
			texture_data[i * 1024 * 4 + j * 4] = texture_data[i * 1024 * 4 + j * 4 + 2];
			texture_data[i * 1024 * 4 + j * 4 + 2] = temp;
		}
	}
}
