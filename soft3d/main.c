...
#define PERFORMANCE_BUFFER_SIZE 16
...
static float performance_ring_buffer[PERFORMANCE_BUFFER_SIZE];
static unsigned int performance_ring_buffer_iterator = 0;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        ...
        case WM_PAINT: {
            ...
            performance_ring_buffer[performance_ring_buffer_iterator] = 1000000.f / ElapsedMicroseconds.QuadPart;
            performance_ring_buffer_iterator = (performance_ring_buffer_iterator + 1) % PERFORMANCE_BUFFER_SIZE;

            float average_performance = 0.f;
            for (size_t i = 0; i < PERFORMANCE_BUFFER_SIZE; i++) {
                average_performance += performance_ring_buffer[i];
            }
            average_performance /= PERFORMANCE_BUFFER_SIZE;

            if (performance_ring_buffer_iterator == 0) {
                char buffer[512] = { 0 };
                sprintf_s(buffer, 512, "soft3d %f fps", average_performance);
